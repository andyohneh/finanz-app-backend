# ki_trainer_swing.py (NEU - für eine mittelfristige Swing-Strategie)
import pandas as pd
import numpy as np
import ta
import joblib
...
def add_features(df: pd.DataFrame) -> pd.DataFrame:
    """Fügt technische Indikatoren für die Swing-Strategie hinzu."""
    print("Füge technische Features für die Swing-Strategie hinzu...")
    # ANGEPASST: Längere Zeitfenster für mittelfristige Trends
    df['sma_fast'] = ta.trend.sma_indicator(df['close'], window=20) # 4 Wochen
    df['sma_slow'] = ta.trend.sma_indicator(df['close'], window=60) # 3 Monate
...
def define_target_and_features(df: pd.DataFrame, future_candles: int = 14, profit_factor: float = 2.5, loss_factor: float = 1.5):
    """
    Definiert das Ziel und die Features für einen Swing-Trade.
    ANGEPASST: Wir schauen 14 Tage in die Zukunft und zielen auf größere Gewinne ab.
    """
...
def train_and_save_model(symbol: str, model_path: str):
    """Steuert den gesamten Trainingsprozess für die Swing-Strategie."""
    ...
    print(f"Starte das Training des Swing-Modells für {symbol}...")
    model = RandomForestClassifier(n_estimators=120, random_state=42, class_weight='balanced', max_depth=8)
...
if __name__ == '__main__':
    print("=== Starte KI-Training für Swing-Strategie ===")
    train_and_save_model('BTC/USD', MODEL_PATH_BTC)
    train_and_save_model('XAU/USD', MODEL_PATH_XAU)
    print("\n=== KI-Training für Swing-Strategie abgeschlossen. ===")