# backtester_daily.py (FINALE VERSION - speichert als backtest_results_daily.json)
import pandas as pd
import numpy as np
import joblib
import os
...
# --- KONFIGURATIONEN ---
MODEL_DIR = "models"; SYMBOLS = ['BTC/USD', 'XAU/USD']
INITIAL_CAPITAL = 100; TRADE_SIZE_USD = 10; TRANSACTION_COST_PERCENT = 0.001
CONFIDENCE_THRESHOLD = 0.75; TREND_SMA_PERIOD = 150 
...
def run_daily_backtest(symbol: str):
    ...
    df['buy_proba'] = probabilities[:, np.where(model.classes_ == 2)[0][0]] # Kaufen ist jetzt Klasse 2
    capital = INITIAL_CAPITAL; position_open = False; trades = []
    ...
    total_return_percent = (capital - INITIAL_CAPITAL) / INITIAL_CAPITAL * 100; num_trades = len(trades)
...
if __name__ == "__main__":
    ...
    if all_results:
        ...
        print("\nErfolgreich 'backtest_results_daily.json' gespeichert.")