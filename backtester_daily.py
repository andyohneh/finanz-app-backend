# backtester_daily.py (DEBUG-VERSION)
print("DEBUG: Skript wird gestartet...")
...
try:
    ...
    print("DEBUG: Kernbibliotheken erfolgreich importiert.")
except Exception as e:
    print(f"!!! FEHLER beim Import der Kernbibliotheken: {e} !!!")
    exit()

try:
    from database import engine, historical_data_daily
    print("DEBUG: Datenbank-Modul erfolgreich importiert.")
except Exception as e:
    print(f"!!! FEHLER beim Import des Datenbank-Moduls: {e} !!!")
    exit()

# --- KONFIGURATION ---
print("DEBUG: Lese Konfiguration...")
MODEL_DIR = "models"
...
print("DEBUG: Konfiguration erfolgreich gelesen.")


def load_data_from_db(symbol: str):
    print(f"DEBUG: Betrete 'load_data_from_db' für Symbol {symbol}.")
    try:
        with engine.connect() as conn:
            query = text("SELECT * FROM historical_data_daily WHERE symbol = :symbol ORDER BY timestamp ASC")
            df = pd.read_sql_query(query, conn, params={'symbol': symbol})
            df['timestamp'] = pd.to_datetime(df['timestamp'])
            print(f"DEBUG: Daten für {symbol} erfolgreich geladen. Anzahl Zeilen: {len(df)}")
            return df
    except Exception as e:
        print(f"!!! FEHLER in load_data_from_db für {symbol}: {e} !!!")
        return pd.DataFrame()

def add_features(df: pd.DataFrame, trend_sma_period: int) -> pd.DataFrame:
    print("DEBUG: Betrete 'add_features' Funktion...")
    ...
    print("DEBUG: 'add_features' Funktion erfolgreich beendet.")
    return df

def run_daily_backtest(symbol: str):
    print(f"DEBUG: Betrete 'run_daily_backtest' für Symbol {symbol}.")
    symbol_filename = symbol.replace('/', '')
    model_path = os.path.join(MODEL_DIR, f'model_{symbol_filename}_swing.pkl')
    print(f"DEBUG: Suche nach Modell unter Pfad: {os.path.abspath(model_path)}")
    if not os.path.exists(model_path):
        print(f"!!! FEHLER: Modell nicht gefunden unter {model_path} !!!")
        return None
    
    print("DEBUG: Modell-Datei gefunden, lade Modell...")
    model = joblib.load(model_path)
    print("DEBUG: Modell geladen.")

    df_full = load_data_from_db(symbol)
    if df_full.empty:
        print("DEBUG: Keine Daten aus DB geladen, breche Backtest ab.")
        return None
    ...
    print("DEBUG: Vorhersagen für die gesamte Historie generiert.")
    ...
    print("DEBUG: Starte Trading-Simulationsschleife...")
    ...
    print("DEBUG: Trading-Simulation abgeschlossen. Berechne KPIs...")
    ...
    print("DEBUG: KPIs berechnet. Erstelle Ergebnis-Dictionary.")
...
if __name__ == '__main__':
    print("DEBUG: Betrete den Hauptausführungsblock (__main__).")
    all_results = []
    print("DEBUG: Starte Schleife über Symbole...")
    for symbol in SYMBOLS:
        print(f"DEBUG: Verarbeite Symbol: {symbol}")
        result = run_daily_backtest(symbol)
        if result:
            all_results.append(result)
            print(f"DEBUG: Ergebnis für {symbol} erfolgreich hinzugefügt.")
        else:
            print(f"DEBUG: Kein Ergebnis für {symbol} erhalten.")

    if all_results:
        print("DEBUG: Alle Backtests abgeschlossen, erstelle DataFrame...")
        ...
        try:
            print("DEBUG: Versuche, JSON-Datei zu speichern...")
